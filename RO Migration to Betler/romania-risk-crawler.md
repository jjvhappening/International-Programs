...
7. **Digest formatter** — reads severity from register; prepends risk ID (e.g. `RR-001`) to each bullet; promotes Escalated risks to the 🚨 section; produces the Slack message with @mentions for Jon and Niels.
   > **Hyperlink rule (mandatory):** Every Jira key that appears as a primary subject in the digest (in bold, as a bullet heading, or in a table cell) MUST be formatted as a markdown hyperlink: `[KEY](https://axilis.atlassian.net/browse/KEY)`. Bare keys like `PLAYER-11` or `GAM-10668` are never acceptable as plain text. Supporting-context keys in description sentences (e.g. `FPT-300`, `TRX-2542`) should also be linked where practical. Run `validate_register.py --digest <file>` to check before posting.
